...
if (! empty ( $_SESSION ['per_login_connecte'] ) && empty ($_POST['vil_num1']) && empty ($_POST['vil_num2'])) {
	// Personne connectée. 
	?>
	<form name='recherche_trajet' id='recherche_trajet' action='#' method='post'>

		<p>
			<label for='vil_num1'>Ville de d&eacute;part :</label> <select name='vil_num1' class='champ'>
		<?php
...
<?php
	...
	<label for='vil_num2'>Ville de d'arriv&eacute;e :</label> <select name='vil_num2' class='champ'>
	<?php
	...
	foreach ( $arrives as $arrive ) {
		...
		<label for='pro_date'>Date de d&eacute;part : </label><input
		type="text" pattern='[0-3][0-9]-[0-1][0-9]-[0-9]{4}' name="pro_date"
		id="pro_date" value="<?php echo date("d-m-Y"); ?>" />
		
		<label for='pro_precision'> Précision : </label> <select name="pro_precision" class='champ'>
		...
		<label
		for='pro_time'> A partir de : </label><input type="text"
		pattern='[0-2][0-9]:[0-5][0-9]' name="pro_time" id="pro_time"
		value="<?php echo date("H:i"); ?>" />
		
		<input type="hidden" name="vil_num1" value=" <?php echo $villeDepart->getVilleNom(); ?> "/>
...
<?php } else	{
	?>Désolé, pas de trajet disponible !<?php 
}
} else	{?>
