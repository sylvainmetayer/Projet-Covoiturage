...
<form action="#" method="POST">
	Personne &agrave; modifier : <select class='champ' name="per_num"
		id="per_num">
				<?php
...
<?php
		...
		?>
<form action="#" method="POST">
	<label for='per_nom'>Nom :</label> <input name='per_nom' class='champ'
		id='per_nom' type='text'
		value="<?php echo $personne->getNomPersonne();?>" required /> <label
		for='per_prenom'>Prenom : </label><input name='per_prenom'
		class='champ' id='per_prenom' type='text'
		value='<?php echo $personne->getPrenomPersonne(); ?>' required /> <br />
	<label for='per_tel'>T&eacute;l&eacute;phone : </label><input
		class='champ' name='per_tel' id='per_tel' type='text'
		value='<?php echo $personne->getPerTel(); ?>' required /> <label
		for='per_mail'> Mail :</label> <input name='per_mail' class='champ'
		id='per_mail' type='text'
		value='<?php echo $personne->getPerMail(); ?>' required /> <br /> <label
		for='per_login'>Login :</label> <input name='per_login' class='champ'
		id='per_login' type='text'
		value='<?php echo $personne->getPerLogin(); ?>' required /> <label
		for='per_mdp'>Mot de passe actuel :</label> <input name='per_mdp'
		class='champ' id='per_mdp' type='password' value='' /> <br />
	...
	<label for='per_nouveau'>Nouveau mot de passe : </label><input
		name='per_nouveau' class='champ' id='per_nouveau' type='password'
		value='' /> <label for='per_confirmation'>Retaper le mot de passe :</label>
	<input name='per_confirmation' class='champ' id='per_confirmation'
		type='password' value='' /> <br /> <input type='submit'
		value='Modifier' />
</form>
<?php
	} else {
